...
// .leerPalabra() - para leer una sola palabra
// .seguir() - para preguntar si seguir o no. lee sólo una s o n, con mensaje personalizado
// .leerChar() - para leer un Character que no sea simbolo o numero -> osea, una letra...
    ...
    class CH
    {
        ...
        public static byte leerOpcion(byte tope)
        {
            ...
            do
            {
                ...
                Console.Write("?> OK... MiK ¿QUÉ DESEAS HACER?: ");
                Console.ForegroundColor = ConsoleColor.Gray;
        ...
        // FUNCIÓN QUE DEVUELVE UNA SÓLA PALABRA (CUALQUIER COSA) QUE NO ESTE VACIO PEDIDO POR TECLADO
        // MOSTRANDO UN MENSAJE PERSONALIZADO POR PANTALLA
        // @param - string: un mensaje para mostrar por pantalla
        public static string leerPalabra(string msg)
        {
            ...
            {   
                ...
                if ((!String.IsNullOrEmpty(aux)))
                {
                    ...
                    if (frase.Length==1) {
                        salir = true;
                        mensaje = "i> OK... PALABRA ACEPTADA!";
                    } else {
                        mensaje = "!> INTRODUCE UNA SÓLA PALABRA, WEBÓN";
                        error = true;
                    }
                }
                else
        ...
        // FUNCION PARA PREGUNTAR SI SEGUIR O NO SEGUIR
        // @param - string: el mensaje a mostrar
        // @return - bool: true si quiere salir false si quiere seguir
        public static bool seguir(string msg) {
            bool salir;
            bool salida;
            string aux;
            int numero;
            ...
            do {
                ...
                {
                    if ( (aux.Length==1) && ( (aux.ToLower().Equals("s")) || (aux.ToLower().Equals("n")) ) )  {
                        if (aux.ToLower().Equals("n")) {
                            salida = true;
                        } else {
                            salida = false;
                        }
        ...
        public static char leerChar(string msg) {
            ...
            do {
                Console.Write("?> "+ msg +": ");
                aux = Console.ReadLine();
                if (aux.ToLower().Equals("resolver")) {
                    salir=true;
                } else {
                    if ((!String.IsNullOrWhiteSpace(aux) && aux.Length==1 && !Int32.TryParse(aux, out numero)))  {
                        if (Char.TryParse(aux,out letra) && Char.IsLetter(letra)) {
                            salir = true;
                        } else {
                            CH.lcdColor("!> Valor NO VÁLIDO! : EL CHAR NO ES UNA LETRA!",ConsoleColor.Red);
                        }
                    } else {
                        CH.lcdColor("!> Valor NO VÁLIDO! : Vacio || EsNumero || Lenght!=1",ConsoleColor.Red);
                    }
                }                
            } while (!salir);
            return letra;
